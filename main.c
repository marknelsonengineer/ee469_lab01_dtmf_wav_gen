...
#define PROGRAM_NAME "ee469_dtmf_wav_gen"
#define FILENAME "/home/mark/src/tmp/blob.wav"

#define CHANNELS 1
#define SAMPLE_RATE 8000       /* Samples per second */
#define BYTES_PER_SAMPLE 1     /* (BitsPerSample * Channels) / 8     1 - 8 bit mono     2 - 8 bit stereo/16 bit mono4 - 16 bit stereo */
#define BITS_PER_SAMPLE 8
#define DURATION_IN_SECONDS 2
#define AMPLITUDE 0.1          /* Max ampltude of signal */
...
void write_header() {
   ...
   uint32_t file_size = 44 + pcm_data_size;  // 44 is the actual size of the header (regardless of what you see below)
   fwrite( &file_size, 1, 4, fp );  /// File size

   fwrite( "WAVE", 1, 4, fp );  /// File type header

   fprintf( fp, "fmt ", 0 );  /// Format chunk marker (includes trailing space)
